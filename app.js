...
app.get("/admindashboard", checkAuth, function (req, res) {
    ...
    con.query("SELECT COUNT(DISTINCT Room_Id) as c FROM studenttable", function (err, result) {
        if (err) throw err;
        rooms_allocated = result[0].c;
        rooms_left = (82-rooms_allocated);
    });
...
app.post("/registration", function (req, res) {
    ...
    if (newStudent.password !== newStudent.confirm_password) {
        ...
        console.log(newStudent.gender.toLowerCase());
        if(newStudent.gender.toLowerCase()==="'male'")
        {
            con.query("Insert into ApplicationTable values("+"Boys Hostel"+","+rollNo+")",(err,res)=>{
                if(err) throw err;
            })
        }
        else if(newStudent.gender.toLowerCase()==="'female'")
        {
            con.query("Insert into ApplicationTable values("+"Girls Hostel"+","+rollNo+")",(err,res)=>{
                if(err) throw err;
            })
        }
    }
...
app.post("/allocateroom", (req, res) => {
    ...
    con.query("SELECT count(*) as c from studentTable where Gender='male' and Disabled='yes' and Room_Id IS NULL", (err, result) => {
        if (err) throw err;
        dis = result[0].c;
        console.log(dis);
    })
    con.query("SELECT count(*) as c from studentTable where Gender='male' and Disabled='no' and Room_Id IS NULL", (err, result) => {
        if (err) throw err;
        nodis = result[0].c;
        console.log(nodis);
    })
    con.query("SELECT count(*) as c from studentTable where Room_Id IS NOT Null", (err, result) => {
        if (err) throw err;
        room = (parseInt(result[0].c/2)+1);
        console.log(room);
        let BlockCapacity = 2;
    let j = 0;
    let flag = 0;
...
app.post('/studentinfo/update', (req, res) => {
    ...
    // Update query
    const sql = `UPDATE studenttable SET FullName = ?, PhoneNo = ?, DOB = ? WHERE RollNo = ?`;
    const values = [name, phoneno, dob, rollno];
    ...
    con.query(sql, values, (err, result) => {
      ...
      console.log(`Student info updated successfully for rollno ${rollno}`);
      // Redirect to the page with the updated data or show success message
      res.redirect('/studentinfo'); // Update this with the appropriate URL
    });
  });
  app.post('/studentinfo/delete', (req, res) => {
    const rollno1 = req.body.rollno1;
    ...
    // Update query
    const sql1 = `Delete from applicationtable WHERE RollNo = ?`;
    const sql2 = `Delete from studenttable WHERE RollNo = ?`;
    const values = [rollno1];
    ...
    con.query(sql1, values, (err, result) => {
      ...
      console.log(`Student info updated successfully for rollno ${rollno1}`);
...
app.post('/studentinfo', function(req, res) {
    ...
    if (sortBy === 'roll') {
        ...
        con.query("select * from studenttable where Room_Id IS NOT NULL ORDER BY RollNo ASC ", function (err, resu) {
            console.log(resu);
            console.log(z);
            students=[];
            for (var x = 0; x < z; x++) {
        ...
        con.query("select * from studenttable where Room_Id IS NOT NULL ORDER BY Room_Id ASC", function (err, resu) {
            console.log(resu);
            console.log(z);
            students=[];
            for (var x = 0; x < z; x++) {
...
let sortedEmployees=[];
app.post("/employee/sort_by_employeeid",function(req,res){
    con.query("use hostelmanagement",(err,res)=>{
        if(err) throw err;
    });
     ...
     con.query("select COUNT(*) as c from employeetable where Emp_Id IS NOT NULL", function (err, res) {
      ...
      con.query("select * from employeetable where Emp_Id IS NOT NULL order by Emp_Id ASC", function (err, resu) {
        console.log(resu);
        console.log(z);
        ...
        res.render("employee", { stud: sortedEmployees});
});

});
let sortedEmployeesbySalary=[];
app.post("/employee/sort_by_salary",function(req,res){
    con.query("use hostelmanagement",(err,res)=>{
        if(err) throw err;
    });
     ...
     con.query("select COUNT(*) as c from employeetable where Emp_Id IS NOT NULL", function (err, res) {
      ...
      con.query("select * from employeetable where Emp_Id IS NOT NULL order by salary ASC", function (err, resu) {
        console.log(resu);
        console.log(z);

        for (var x = 0; x < z; x++) {
            sortedEmployeesbySalary.push({
                a: resu[x].Emp_Id,
                b: resu[x].EName,
                c: resu[x].Address,
                d: resu[x].Salary,
        ...
        res.render("employee", { stud: sortedEmployeesbySalary});
});

});
let DeletedEmployee=[]
app.post("/employee/deletebyEmpId",function(req,res){
    let Empid=req.body.eight;
    con.query("use hostelmanagement",(err,res)=>{
        if(err) throw err;
    });
    con.query("delete from employeetable where Emp_Id="+Empid,(err,res)=>{
        if(err) throw err;
    })
     con.query("select COUNT(*) as c from employeetable where Emp_Id IS NOT NULL", function (err, res) {
        console.log(res);
      ...
      con.query("select * from employeetable where Emp_Id IS NOT NULL order by salary ASC", function (err, resu) {
        ...
        res.render("employee", { stud: DeletedEmployee});
});
